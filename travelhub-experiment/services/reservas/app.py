...
def random_failure():
    ...
    if roll < 0.3:
        ...
        logger.info(f"reservas -> DEGRADED (delay: {service_state['delay']:.1f}s)")
    ...
    # En caso de que el número sea menor a 0.5
        ...
        # Imprimir mensaje de log
        logger.info(f"reservas -> UNHEALTHY")
    ...
    # Con un roll superior a 0.5
        ...
        # Imprimir mensaje de log
        logger.info(f"reservas -> HEALTHY")
...
def health():
    ...
    if service_state["status"] == "unhealthy":
        return jsonify({"service": "reservas", "status": "unhealthy", "timestamp": datetime.now(pytz.timezone('America/Bogota')).strftime('%Y-%m-%d %H:%M:%S %Z')}), 500
    ...
    return jsonify({"service": "reservas", "status": service_state["status"], "timestamp": datetime.now(pytz.timezone('America/Bogota')).strftime('%Y-%m-%d %H:%M:%S %Z')}), 200
