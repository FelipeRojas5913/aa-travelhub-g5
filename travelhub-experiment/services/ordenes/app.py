...
def random_failure():
    ...
    if roll < 0.3:
        ...
        logger.info(f"ordenes -> DEGRADED (delay: {service_state['delay']:.1f}s)")
    ...
    # En caso de que el número sea menor a 0.5
        ...
        # Imprimir mensaje de log
        logger.info(f"ordenes -> UNHEALTHY")
...
def health():
    ...
    if service_state["status"] == "unhealthy":
        return jsonify({"service": "ordenes", "status": "unhealthy", "timestamp": datetime.now(pytz.timezone('America/Bogota')).strftime('%Y-%m-%d %H:%M:%S %Z')}), 500
    ...
    return jsonify({"service": "ordenes", "status": service_state["status"], "timestamp": datetime.now(pytz.timezone('America/Bogota')).strftime('%Y-%m-%d %H:%M:%S %Z')}), 200
