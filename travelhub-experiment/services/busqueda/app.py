...
def health():
    ...
    if service_state["status"] == "unhealthy":
        return jsonify({"service": "busqueda", "status": "unhealthy"}), 500
    ...
    # En caso de que se encuentre "healthy", se devuelve un código de estado 200
        ...
        return jsonify({"service": "busqueda", "status": service_state["status"], "timestamp": datetime.now(timezone.utc)}), 200
